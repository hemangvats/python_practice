...
def add():
    ...
    try:
        ...
        l={'i':BId,'a':Arrival,'d':Destination,'j':DOJ,'f':fare}
        pickle.dump(l,f)
    except:
...
def modify(BId,old):
    ...
    try:
        while(f):
            ...
            if BId==l['i']:
                if old==2:
                    new=input('Enter new time for departure:')        
                    l['a']=new
                elif old==3:
                    new=input('Enter the new destination:')
                    l['d']=new
                elif old==4:
                    new=input('Enter new date of departure:')
                    l['j']=new
                else:
                    new=float(input('Enter new fare for the journey:'))
                    l['f']=new
                pickle.dump(l,f1)
            else:
                pickle.dump(l,f1)
